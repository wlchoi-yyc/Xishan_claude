...
/**
 * 建立人物。正面朝 +z。
 * opts: { robe, inner, skin, hair, beard, cap: 'scholar'|'band'|'hat'|null, scale }
 */
export function makePerson(opts = {}) {
  const o = Object.assign({ robe: '#9aa7a6', inner: '#e8e1cf', skin: '#e0b894', hair: '#1c1a18', beard: false, cap: 'scholar', belt: '#4b3b2e', scale: 1 }, opts);
  const root = new THREE.Group();
  const body = new THREE.Group(); root.add(body);
  const robeM = lam(o.robe), skinM = lam(o.skin), hairM = lam(o.hair), innerM = lam(o.inner), beltM = lam(o.belt);

  // 站姿下半身（長袍）
  const skirt = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.36, 1.02, 8), robeM);
  skirt.position.y = 0.51; body.add(skirt);
  // 坐姿下半身
  const lap = new THREE.Group(); lap.visible = false; body.add(lap);
  const hip = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.36, 0.3, 8), robeM); hip.position.y = 0.15; lap.add(hip);
  const legL = new THREE.Mesh(new THREE.CapsuleGeometry(0.11, 0.5, 3, 6), robeM); legL.rotation.x = Math.PI / 2 - 0.25; legL.position.set(0.14, 0.2, 0.36); lap.add(legL);
  const legR = legL.clone(); legR.position.x = -0.14; lap.add(legR);
  const feet = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.1, 0.16), lam('#2b2622')); feet.position.set(0, 0.06, 0.68); lap.add(feet);

  const upper = new THREE.Group(); body.add(upper);
  const torso = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.25, 0.58, 8), robeM); torso.position.y = 1.3; upper.add(torso);
  const collar = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.2, 0.12, 8), innerM); collar.position.y = 1.58; upper.add(collar);
  const belt = new THREE.Mesh(new THREE.CylinderGeometry(0.255, 0.255, 0.07, 8), beltM); belt.position.y = 1.06; upper.add(belt);

  function arm(side) {
    const g = new THREE.Group(); g.position.set(0.26 * side, 1.52, 0);
    const sleeve = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.14, 0.6, 6), robeM); sleeve.position.y = -0.3; g.add(sleeve);
    const hand = new THREE.Mesh(new THREE.SphereGeometry(0.055, 6, 5), skinM); hand.position.y = -0.63; g.add(hand);
  ...
  const head = new THREE.Group(); head.position.y = 1.72; upper.add(head);
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 0.1, 6), skinM); neck.position.y = -0.1; head.add(neck);
  const face = new THREE.Mesh(new THREE.SphereGeometry(0.135, 12, 10), skinM); face.position.y = 0.06; face.scale.set(1, 1.12, 1); head.add(face);
  const hairBack = new THREE.Mesh(new THREE.SphereGeometry(0.142, 12, 10, 0, Math.PI * 2, 0, Math.PI * 0.55), hairM);
  hairBack.position.set(0, 0.075, -0.012); hairBack.rotation.x = -0.35; head.add(hairBack);
  const eyeM = lam('#16120f');
  for (const s of [1, -1]) {
    const eye = new THREE.Mesh(new THREE.SphereGeometry(0.018, 6, 5), eyeM); eye.position.set(0.048 * s, 0.085, 0.122); head.add(eye);
    const brow = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.01, 0.01), hairM); brow.position.set(0.05 * s, 0.12, 0.125); brow.rotation.z = -0.15 * s; head.add(brow);
  }
  const nose = new THREE.Mesh(new THREE.ConeGeometry(0.018, 0.05, 4), skinM); nose.rotation.x = Math.PI / 2; nose.position.set(0, 0.05, 0.145); head.add(nose);
  const mouth = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.008, 0.01), lam('#8a4a3a')); mouth.position.set(0, 0.0, 0.13); head.add(mouth);
  if (o.beard) {
    const beard = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.2, 6), hairM); beard.rotation.x = Math.PI; beard.position.set(0, -0.1, 0.09); head.add(beard);
    for (const s of [1, -1]) {
      const mu = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.012, 0.012), hairM); mu.position.set(0.03 * s, 0.018, 0.135); mu.rotation.z = 0.35 * s; head.add(mu);
    }
  }
  if (o.cap === 'scholar') {
    const cap = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.14, 0.22), lam('#1a1816')); cap.position.set(0, 0.2, -0.01); head.add(cap);
    const bun = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.12, 0.12), lam('#1a1816')); bun.position.set(0, 0.3, -0.03); head.add(bun);
    for (const s of [1, -1]) {
      const tail = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.24, 0.01), lam('#1a1816')); tail.position.set(0.06 * s, 0.08, -0.15); tail.rotation.x = 0.25; head.add(tail);
    }
  } else if (o.cap === 'band') {
    const bun = new THREE.Mesh(new THREE.SphereGeometry(0.07, 6, 5), hairM); bun.position.set(0, 0.22, -0.02); head.add(bun);
    const band = new THREE.Mesh(new THREE.TorusGeometry(0.14, 0.018, 4, 12), lam('#6b4b36')); band.rotation.x = Math.PI / 2; band.position.y = 0.13; head.add(band);
  } else if (o.cap === 'hat') {
    const hat = new THREE.Mesh(new THREE.ConeGeometry(0.42, 0.22, 12), lam('#c8a86a')); hat.position.y = 0.26; head.add(hat);
  }
  root.scale.setScalar(o.scale);

  const ud = root.userData;
  Object.assign(ud, { head, armL, armR, upper, skirt, lap, body, pose: 'stand', walkT: 0, walking: false });

  ud.setPose = (pose) => {
    ud.pose = pose;
    body.rotation.set(0, 0, 0); body.position.set(0, 0, 0);
    skirt.visible = true; lap.visible = false; upper.position.set(0, 0, 0); upper.rotation.set(0, 0, 0);
    armL.rotation.set(0, 0, 0.12); armR.rotation.set(0, 0, -0.12);
    if (pose === 'sit') {
      skirt.visible = false; lap.visible = true;
      upper.position.y = -0.78;
      armL.rotation.set(-0.7, 0, 0.1); armR.rotation.set(-0.7, 0, -0.1);
    } else if (pose === 'lie') {
      body.rotation.x = -Math.PI / 2; body.position.set(0, 0.3, 0.9);
    }
  };
  ud.update = (dt) => {
    if (ud.walking) {
      ud.walkT += dt * 7;
      armL.rotation.x = Math.sin(ud.walkT) * 0.4;
      armR.rotation.x = -Math.sin(ud.walkT) * 0.4;
      body.position.y = Math.abs(Math.sin(ud.walkT)) * 0.04;
    } else if (ud.pose === 'stand' && !ud.customArms) {
      armL.rotation.x *= 0.9; armR.rotation.x *= 0.9;
      body.position.y = Math.sin(E.time * 1.6 + root.id) * 0.004;
    }
